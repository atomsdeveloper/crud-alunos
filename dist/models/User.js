 ...
 class User extends _sequelize.Model {
  static init(sequelize) {
    super.init(
      {
        name: {
          type: _sequelize.DataTypes.STRING(100), // String type with a maximum length of 100
        ...
        password: {
          type: _sequelize.DataTypes.VIRTUAL, // Virtual field
          allowNull: false, // Not null
          defaultValue: "", // Default value is an empty string
          validate: {
            notNull: {
    ...
    this.addHook("beforeSave", async (user) => {
      ...
      if (user.password) {
        user.password_hash = await _bcryptjs2.default.hash(user.password, 8); // Hashing the password with a salt rounds of 8
      }
    });
    return this;
  }
